    ...
    match source {
        Some(s) => {
            ...
            if path.is_file() {
                ...
                // Path doesn't exist yet — treat as intended file in parent dir
                let parent = path.parent().unwrap_or(&path).to_path_buf();
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Also emit after frontend is ready (the window may not have loaded yet)
            let payload_clone = payload.clone();
            let handle = app.handle().clone();
            std::thread::spawn(move || {
                // Small delay to let the frontend initialize its event listeners
